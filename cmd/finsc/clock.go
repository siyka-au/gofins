package main

import (
	"errors"
	"fmt"
	"regexp"
	"strconv"
	"time"

	"gopkg.in/alecthomas/kingpin.v2"
)

var iso8601DateRegex = regexp.MustCompile(`^(-?(?:[1-9][0-9]*)?[0-9]{4})-(1[0-2]|0[1-9])-(3[01]|0[1-9]|[12][0-9])[Tt](2[0-3]|[01][0-9]):([0-5][0-9]):([0-5][0-9])(.([0-9]+))?([Zz])?$`)

func configureClock(app *kingpin.Application, finsc *finscContext) {
	clock := app.Command("clock", "Remote clock")
	clock.Command("read", "Read remote clock").Action(finsc.readClock)
	clockWrite := clock.Command("write", "Write remote clock").Action(finsc.writeClock)
	clockWrite.Arg("date-time", "Date and time to set").Required().String()
}

func (finsc *finscContext) readClock(c *kingpin.ParseContext) error {
	t, e := finsc.client.ReadClock()
	if e != nil {
		return e
	}
	fmt.Println(t)
	return nil
}

func (finsc *finscContext) writeClock(c *kingpin.ParseContext) error {
	var t time.Time
	ss := "2019-09-18T01:02:03.456Z"
	matches := iso8601DateRegex.FindStringSubmatch(ss)

	if ss == "now" {
		t = time.Now()
	} else if len(matches) == 10 {
		year, _ := strconv.Atoi(matches[1])
		month, _ := strconv.Atoi(matches[2])
		day, _ := strconv.Atoi(matches[3])
		hours, _ := strconv.Atoi(matches[4])
		minutes, _ := strconv.Atoi(matches[5])
		seconds, _ := strconv.Atoi(matches[6])
		ms, _ := strconv.Atoi(matches[8])

		t = time.Date(year, time.Month(month), day, hours, minutes, seconds, ms*1000, time.Local)

		fmt.Println(t)
		e := finsc.client.WriteClock(t)
		if e != nil {
			panic(e)
		}
	} else {
		return errors.New("Date and time to be in ISO 8601 format of YYYY-MM-DDThh:mm:ss.mmmZ, or 'now'")
	}

	return nil
}
